...
def calibrate_camera(images, chessboardSize, square_size):
    ...
    for i, corners2 in enumerate(imgpoints):
        _, rvec, tvec = cv.solvePnP(objpoints[i], corners2, cameraMatrix, dist) #single image pose 
...
def analyze_errors_by_distance(distances, errors):
    plt.scatter(distances, errors, c='blue', marker='o', label='Reprojection Error by Distance (mm)')
...
cameraMatrix, dist, rvecs, tvecs, distances,objpoints, imgpoints = calibrate_camera(all_images, chessboardSize, size_of_chessboard_squares_mm)

errors, mean_error = calculate_reprojection_error(objpoints, imgpoints, cameraMatrix, rvecs, tvecs, dist)
...
undistorted_img = undistort_image('links_8bit_jpg_DS_0.5/DSC00123_DS_0.5.jpg', cameraMatrix, dist)

def measure_distances(image, chessboardSize):
    ...
    distances_matrix = np.zeros((chessboardSize[0] - 1, chessboardSize[1] - 1))

    for i in range(chessboardSize[0] - 1):
        for j in range(chessboardSize[1] - 1):
            pt1 = corners2[i * chessboardSize[1] + j][0]
            pt2 = corners2[i * chessboardSize[1] + (j + 1)][0]
            distance = np.linalg.norm(pt1 - pt2)
            distances_matrix[i, j] = distance

    return distances_matrix

distances_matrix = measure_distances(undistorted_img, chessboardSize)
print("Measured distances:", distances)

if distances_matrix is not None:
    plt.imshow(distances_matrix, cmap='coolwarm', interpolation='nearest')
    plt.colorbar()
    plt.title('Distance Variation Across Chessboard Corners')
    plt.xlabel('Corner Index Along Width')
    plt.ylabel('Corner Index Along Height')
    plt.show() """